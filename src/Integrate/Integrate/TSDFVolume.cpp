...
TSDFVolume::TSDFVolume( int cols, int rows, std::string camera_file )
	: cols_( cols )
	, rows_( rows )
	...
	, integration_trunc_(4.0)
{
	...
	integration_trunc_ = camera_.integration_trunc_;
}
...
#pragma omp parallel for num_threads( 8 )
	for ( int y = 0; y < rows_; y++ ) {
		for ( int x = 0; x < cols_; x++ ) {
			...
			float res = d * lambda / camera_.depth_ratio_;
			if ( res > integration_trunc_ ) {
				scaled[ y * cols_ + x ] = 0.0f;
			} else {
				scaled[ y * cols_ + x ] = res;
			}
		}
	}
}
	...
	for ( int v = 0; v < rows_; v += 1 ) {
		for ( int u = 0; u < cols_; u += 1 ) {
			...
			if ( UVD2XYZ( u, v, d, x, y, z ) ) {
				...
				if (xi >= 512 || xi<0 || yi >= 512 || yi<0 || zi >= 512 || zi<0)
				{
					assert(true && "WARNING: The model is too large\n");
				}
